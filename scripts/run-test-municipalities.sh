...
if [ -z "$DB_URL" ]; then
  ...
  echo "Example usage: ./scripts/run-test-municipalities.sh \"postgresql://username:password@localhost:5432/kpi_edge_db\""
else
  echo "🔍 Testing municipality data"
  npx tsx scripts/test-municipalities.ts --database-url="$DB_URL"
fi
